(function (global) {
  ...
  function normalizeLog(log, index) {
    ...
    const logNumber = String(sourceLog.logNumber || id).padStart(3, '0');
  ...
  function sortLogsNewestFirst(logs) {
    return [...logs].sort((first, second) => {
      const firstDate = `${first.date || ''}-${String(first.logNumber || first.id || 0).padStart(6, '0')}`;
      const secondDate = `${second.date || ''}-${String(second.logNumber || second.id || 0).padStart(6, '0')}`;
  ...
  global.B20Data = {
    async loadLogs() {
      ...
      const logs = sortLogsNewestFirst(source.map(normalizeLog));
    ...
    getLatestLog(logs) {
      return asArray(logs, [])[0] || null;
    },

    getFeaturedLog(logs) {
      const safeLogs = asArray(logs, []);
      return safeLogs.find((log) => log.featured) || safeLogs[0] || null;
    }
  };
})(window);
